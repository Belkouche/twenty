...
export class RecordSharingService {
  ...
  async getSharing(args: RecordSharingArgs): Promise<RecordSharingDTO> {
    ...
    const shares = permissions.canUpdate
      ...
      : [];
    const { flatRoleMaps } = permissions.canUpdate
  ...
  async setShare(
    ...
    if (
      ...
      args.accessLevel !== RecordShareAccessLevel.READ_WRITE
    ) {
      throw new UserInputError('Manual sharing supports read or write access');
    }
