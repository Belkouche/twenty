...
const fields =
  'permissions { canRead canUpdate canDelete canSoftDelete } isEnabled hasInheritedAccess roles { id } shares { principalId rowCause accessLevel }';
...
describe('Generic sharing API on an ordinary private object', () => {
  ...
  const change = (
    ...
    makeMetadataAPIRequest(
      { query: SET_SHARE, variables: { target: target(), principal, enabled } },
      token,
    );
  const read = () =>
  ...
  it('grants edit access through the reusable API while preserving role restrictions and rejecting full access', async () => {
    const principal = {
  ...
  it('lets a non-owner writer manage sharing while preserving independent destructive permissions', async () => {
    ...
    const updated = await makeGraphqlAPIRequest(
      updateOneOperationFactory({
        ...
        gqlFields: 'id',
        recordId: RECORD_ID,
        data: { name: 'Updated by writer' },
      }),
      APPLE_JONY_MEMBER_ACCESS_TOKEN,
    );
    expect(updated.body.errors).toBeUndefined();
    const result = await change({ everyone: true }, true);
    expect(result.body.errors).toBeUndefined();
    expect(result.body.data.setRecordShare.permissions).toEqual({
      canRead: true,
      canUpdate: true,
      canDelete: false,
      canSoftDelete: false,
    });
    expect(result.body.data.setRecordShare.shares).toEqual(
