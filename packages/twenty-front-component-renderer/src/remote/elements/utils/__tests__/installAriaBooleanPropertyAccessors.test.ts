...
describe('installAriaBooleanPropertyAccessors', () => {
  ...
  it('should not define prototype accessors for aria attributes that remote-dom declares as properties', () => {
    const prototype = Object.getPrototypeOf(createHtmlDivElement());

    expect(
      Object.getOwnPropertyDescriptor(prototype, 'aria-hidden'),
    ).toBeUndefined();
    expect(
      Object.getOwnPropertyDescriptor(prototype, 'aria-invalid')?.set,
    ).toBeDefined();
  });
});
