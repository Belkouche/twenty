...
|----------------|--------------|-------------|
| `ref.current.focus()`, `.blur()` | Move the sandbox's `document.activeElement` only; the real focus on the page does not change | `onFocus` / `onBlur` for the page's focus |
| `ref.current.click()`, `.select()`, `.setSelectionRange()`, `.scrollIntoView()`, `video.play()` | Throws | Controlled components; read values from `event.target` |
...
| `document.activeElement` | Reflects `focus()` / `blur()` calls made inside the sandbox, plus the page's focus for elements that have an `onFocus` / `onBlur` handler | Track the page's focus with `onFocus` / `onBlur` |
